package org.anasoid.jmixjpaodatademo.odata.jpa.processor.core.api;
...
public class JPACUDRequestHandler extends JPAExampleCUDRequestHandler {
    ...
    public void validateChanges(final EntityManager em) throws ODataJPAProcessException {
        ...
        for (final Map.Entry<Object, JPARequestEntity> entity : entityBuffer.entrySet())
    ...
    private void processBindingLinks(final Map<JPAAssociationPath, List<JPARequestLink>> relationLinks,
        ...
        for (final Map.Entry<JPAAssociationPath, List<JPARequestLink>> entity : relationLinks.entrySet()) {
    ...
    private void updateLinks(final JPARequestEntity requestEntity, final EntityManager em, final Object instance)
        ...
        if (requestEntity.getRelationLinks() != null)
            for (final Map.Entry<JPAAssociationPath, List<JPARequestLink>> links : requestEntity.getRelationLinks().entrySet()) {
    ...
    private boolean hasGeneratedKeyInt(final JPAEntityType et, final EntityType<?> jpaEt) {
        try {
            if (jpaEt.hasSingleIdAttribute()) {
                ...
                if ("integrationKey".equalsIgnoreCase(key.getExternalName())) {
                    return true;
                }
